defmodule Lms.Accounts.User do
  ...
  @doc """
  A changeset for updating a user's role and company association.
  """
  def role_changeset(user, attrs) do
    user
    |> cast(attrs, [:role, :company_id])
    |> validate_inclusion(:role, @roles)
    |> validate_company_required()
    |> foreign_key_constraint(:company_id)
  end

  defp validate_company_required(changeset) do
    role = get_field(changeset, :role)
    company_id = get_field(changeset, :company_id)

    if role != :system_admin && is_nil(company_id) do
      add_error(changeset, :company_id, "is required for non-system admin users")
    else
      changeset
    end
  end
  ...
  """
  def valid_password?(%Lms.Accounts.User{hashed_password: hashed_password}, password)
